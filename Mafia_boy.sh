...
if [[ "$1" == "--remove" ]]; then
	...
	rm -rf ~/.oh-my-zsh ~/.plugins;	chsh -s bash;
...
if [[ ${PROC32} == [Y/y] ]]; then
	...
	if [ -d $HOME/P-Mafia ]; then

	cd $HOME/P-Mafia

	fi

#if [ -e $HOME/.Mafia ]; then
...
echo -e "\033[31m$(figlet -f Remo773 "P- Mafia")\e[0m"

echo -e "\e[1;32m

+----------------------------------*/

ZihadVau99 : (\e[33m14.4.2021\e[32m)
